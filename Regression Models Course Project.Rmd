---
title: "Effects of Transmission Type on MPG of Cars"
author: "Clay Burns"
date: "June 20, 2015"
output: pdf_document
---
## Executive Summary
In this report data gathered by Moto Trends is explored and analyzed in order to determine the relationship between the collected variables and miles per gallon of automobiles. In particlular, the following two issues will be addressed:
...
A boxplot was produced to show the difference between automatic and manual in terms of MPG. In exhibit 1, it is clear that manual transmission produces more MPG. Next, a pairwise graph (exhibit 2) was created in order to get a greater intuition of what other variables may be of interest. There is a linear relationship between MPG and each of cyl, disp, hp, drat, wt, qsec, vs, am. The covariance was also computed (exhibit 3) between every variable and the positive values were noted (qsec = 0.419, vs = 0.664, am = 0.600, gear = 0.480). Then a linear model was fit on all the variables to determine which variables should be used in the final models. In figure 4 the summary from this model is shown. The lowest p values were taken (i.e. wt = 0.063, am = 0.234, qsec = 0.274) due to their high significance in predicting MPG.
...
- The Residuals vs Fitted plot shows no pattern between the residuals and fitted values indicating that this regression model is well constructed.   
- The QQ plot shows that the points line up as expected meaning that the distribtion is normal and our model predictions should be accurate.  
- In the Scale-Location plot and the Residuals vs Leverage plots, the points are in a group not far from the center indicating no point had too much leverage.

## Statistical Inference
A Two Sample t-test was conducted between the different transmission types. The null hypothesis that transmission types don't have an effect on the MPG is discarded for a p-value greater than 0.05. The results are shown in figure 8. The p-value of 0.001374 and difference in mu show that manual transmission has significantly more MPG than automatic.
```{r}
...
## Conclusions
The transmission type of a car has a significant effect on its fuel efficiency. According to the model, manual transmission, on average, has 1.81 MPG more than automatics. According to the boxplot, manual transmission are even more MPG efficent but this is likely due to smaller weights likely in manual sports cars. 
